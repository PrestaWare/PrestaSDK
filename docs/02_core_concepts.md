...
To create a page in the admin area, you simply need to create a controller that extends PrestaSDK\V072\Controller\AdminController. This base class automatically uses a Trait called PanelCore, which contains all the logic for rendering the panel, managing sections, and handling templating.
...
To solve this, all PrestaSDK classes are placed within a versioned namespace. For example, in version 0.4.0, all classes reside under PrestaSDK\V072:

```php
namespace PrestaSDK\V072;
...
This structure ensures that if a future version 0.7.2 is released with the namespace PrestaSDK\V072, its code will not conflict with older versions.
...
If you decide to upgrade the SDK version in your module in the future, you must manually update your use statements to the new version (e.g., PrestaSDK\V072\...).
#### PrestaSDKFactory
The PrestaSDKFactory class is a helper tool to simplify the process of creating instances of SDK classes within the same version. Instead of directly calling new \PrestaSDK\V072\Utility\Config(...), you can use the Factory for more readable code.

```php
use PrestaSDK\V072\PrestaSDKFactory;

// This Factory creates an instance of the Config class from the V072 namespace
